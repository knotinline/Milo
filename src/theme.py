...
[data-testid="stPageLink"] a:hover {
    background: rgba(103, 213, 255, 0.22);
}
...
.logo-icon {
    filter: drop-shadow(0 0 8px rgba(103, 213, 255, 0.55));
...
.sidebar-card,
div[class*="st-key-sidebar_settings"] {
...
div[data-baseweb="base-input"] {
    border-color: rgba(103, 213, 255, 0.4) !important;
...
def html_block(content: str) -> None:
    """Render multi-line HTML via st.markdown, stripping per-line indentation.

    Markdown treats 4+ leading spaces on a line as a preformatted code block,
    which escapes its contents instead of parsing them as HTML. Since these
    blocks get embedded inside indented Python code (and can nest generated
    HTML, like the owl SVG, on top of that), indentation easily creeps past
    that threshold and breaks rendering.
    """
    st.markdown("\n".join(line.strip() for line in content.strip().splitlines()), unsafe_allow_html=True)
