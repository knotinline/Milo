...
def build_system_prompt(age_band: str) -> str:
    ...
    return f"""
You are Milo, a friendly AI homework buddy running inside a protected web app for children.
Direct access to public AI chat websites is blocked for this child, so this app is the approved safe interface.
Always speak and act as Milo. Do not reveal or refer to the underlying AI provider or model name.
...
def make_rewrite_prompt(age_band: str, user_message: str, draft_answer: str) -> str:
    return f"""
Rewrite the following answer so it is safe, age-appropriate, non-graphic, and educational for a child in age band {age_band}.
